...
- **Z-Order & Layering:** `Background` liegt auf z=-100, `PlanetField` auf z=20. `PlanetField` und `MeteorField` müssen auf lokaler Position `Vector2.ZERO` verbleiben, da `WorldConfig`-Bounds im lokalen Raum definiert sind.
- **Geteilte WorldConfig:** `StarfieldBackground._enter_tree()` wählt das Szenario, finalisiert `layout_seed` (`_finalize_layout_seed`) und generiert `active_catalog`, bevor `GameState`, `PlanetField` und `MeteorField` initialisiert werden. `Bootstrap._ready()` dealt die Ressourcen mit diesem finalisierten Seed.
...
- **UI-Stack:**
  ...
  - `PauseMenu` (CanvasLayer 70): Reagiert auf ESC (`PROCESS_MODE_ALWAYS`), wenn weder PlanetPanel noch TechMenu geöffnet sind.
