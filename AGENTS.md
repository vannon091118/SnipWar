...
- Headless uses the Dummy renderer, so `Window.get_texture()` is null and cannot produce visual captures; visual regression screenshots must run with the desktop Compatibility/OpenGL renderer.
- For visual tests, set `Window.size` again after the scene is initialized; the project window override can otherwise leave the first 960×540 capture at the physical override size.
...
- `PlanetField` and `MeteorField` must remain at local position zero in the main background scene; WorldConfig bounds are already authored in their local world space, so editor offsets shift or clip the whole map.
...
- `NavigationConfig` consumes a typed `NavigationWaypointCatalog`; each map may provide a different catalog/cadence, so navigation configs, map resources, scenario selection, and preflight must change together.
...
- Scenario selection is applied by the Background root in `_enter_tree()` before PlanetField enters its tree; applying a scenario from Bootstrap `_ready()` is too late for catalog generation. Bootstrap only randomizes the seed when `ScenarioDefinition.randomize_layout_seed` is true.
- `ScenarioCatalog` currently selects data for a fresh scene instance; `_catalog_generated` means changing the catalog on an active PlanetField does not hot-swap planets.
...
- The satellite detail intentionally uses `angular_speed_range = Vector2(0.18, 0.18)` for a uniform visual cadence; treat it as a fixed style value unless a deliberate variation decision changes the resource contract.
...
- Navigation changes span `scripts/config/navigation_config.gd`, the waypoint definition/catalog scripts and `resources/config/navigation_waypoints/*`, `resources/config/navigation_default.tres`, scenario-specific navigation resources, `scripts/objects/planets/navigation_field.gd`, `scripts/objects/planets/navigation_waypoint.gd`, its scene/SVG import sidecars, `seeded_layout.gd`, `planet_network.gd`, `worker_manager.gd`, `transit_config.gd`, `flight_time.gd`, and `preflight.gd`; update them as one pathing contract.
...
- Phase-4 scenario changes span `scripts/config/map_definition.gd`, `scripts/config/scenario_definition.gd`, `scripts/config/scenario_catalog.gd`, `resources/config/maps/*`, `resources/config/scenarios/*`, `resources/config/scenario_catalog.tres`, `resources/config/world_wide.tres`, `starfield_background.gd`, `bootstrap.gd`, `starfield_background.tscn`, `preflight.gd`, and `DESIGN.md`; keep map-resource references and pre-tree application synchronized.
...
- ScenarioDefinition owns the active route rule (`default=all_planets`, `wide=neighbors_only`) and applies it to the shared WorldConfig before PlanetField enters the tree; keep scenario and map routing values synchronized.
...
- Visual regression of routes must activate the PlanetNetwork UI before capture; otherwise `_draw()` intentionally omits the selected route line. Capture the transit frame immediately after dispatch for the exact launch formation, then wait only for movement assertions.
...
- A `MultiMeshInstance2D` requires `MultiMesh.mesh` (a `QuadMesh`) before `instance_count`; a texture and transforms alone trigger the Compatibility renderer's misleading `mesh.is_null()` errors.
