...
- **CLI-Optionen:** Die Preflight-Suite unterstützt `--verbose` / `-v` (Detail-Assertions), `--fail-fast` / `-x` (sofortiger Abbruch bei erstem Fehler), `--filter=<name>` / `-f=...` (gezielte Constraint-Filterung mit automatischem Scene-Boot falls nötig), `--reverse` (Reverse-Execution) und `--list` / `-l` (Übersicht aller 34 Constraints).
