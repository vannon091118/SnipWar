...
| Dokument | Pfad | Zweck & Inhalt |
|:---|:---|:---|
| **Agenten- & Entwicklerregeln** | [`AGENTS.md`](../AGENTS.md) | **Faktengetreue operative Leitlinien:** Godot-Headless-Regeln, modulares Preflight-Design (19 Constraints, `PreflightFixture`-Isolation, Seed `424242`), atomare Commit-Gruppen (`Change Together`), Godot-Fallstricke und unumgehbare Git-Hook-Kette. |
...
3. **Preflight-Konsistenz:** Die Anzahl der Constraints (aktuell **19 Constraints**) und die CLI-Flags (`--verbose`, `--fail-fast`, `--filter`, `--reverse`, `--list`) müssen über alle Dokumente einheitlich geführt werden.
