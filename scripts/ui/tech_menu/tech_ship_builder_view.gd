...
func populate_builder_dynamic(state: Node, on_refresh_callback: Callable) -> void:
	...
	if not build_jobs.is_empty():
		...
		for job_ship_value in build_jobs:
			...
			var job_label := UIBaseUtils.make_label("Montage läuft — %s (%.0f s verbleibend)" % [_assembly_role(build_jobs[job_ship_id]), remaining], _theme_config.muted_text_color, _theme_config.small_font_size)
			job_label.tooltip_text = _assembly_tooltip(catalog, build_jobs[job_ship_id], remaining)
	...
	if assemblies.is_empty():
		...
		for ship_value in assemblies:
			...
			var assembly_role: StringName = assemblies[ship_id].get("role", &"colony") as StringName
...
func _assembly_role(assembly: Dictionary) -> String:
	var role: String = String(assembly.get("role", "colony"))
	if role == "military":
		return "MILITÄR"
	if role == "colony":
		return "KOLONIE"
...
func _assembly_tooltip(catalog: ShipPartCatalog, assembly: Dictionary, remaining: float) -> String:
	...
	for slot_name in [&"hull", &"drive", &"weapon", &"shield", &"scanner"]:
		...
		if part == null:
			continue
		var variant_id: StringName = (assembly.get("variants", {}) as Dictionary).get(slot_name, &"") as StringName
		var variant: ShipComponentVariant = catalog.resolve_variant(part, variant_id)
		var combined_trait: TraitDefinition = catalog.combined_trait(part, variant)
		var line: String = "%s: %s" % [String(slot_name).capitalize(), part.display_name]
		if variant != null:
