...
func search(query: String) -> Array[ConceptEntry]:
	...
	for concept_value in _concepts.values():
		...
		var score: int = _score_entry(entry, normalized)
		if score > 0:
			result.append({"entry": entry, "score": score})
...
func _build_concepts() -> void:
	...
	_add_concept("economy_resources", "economy", ["EconomyDomain", "EconomyConfig", "EconomyWindow", "ResourcePool", "GameResource", "BuildingCatalog", "PlanetUpgradeCatalog"], ["add_faction_resource", "spend_faction_resource", "get_faction_resource", "purchase_upgrade", "place_planet_building", "get_local_resource"], "Vaults, resources, upgrades, buildings and trade", ["economy", "ökonomie", "resource", "ressource", "money", "geld", "credit", "vault", "upgrade", "building", "trade", "handel"])
	_add_concept("workers_transit", "transit", ["WorkerCluster", "FlightTime", "TransitConfig", "TransitRecord"], ["begin_worker_transport", "update_worker_transport", "complete_worker_transport", "get_worker_transport_records"], "Worker transit, cluster packing and flight time", ["worker", "arbeiter", "transport", "transit", "cluster", "flight", "flug", "flight time"])
	_add_concept("navigation_world", "navigation", ["NavigationField", "NavigationWaypoint", "NavigationConfig"], ["find_route", "get_neighbors", "build_graph"], "Routes, waypoints and navigation graph", ["navigation", "routing", "route", "pfad", "waypoint", "graph", "neighbor", "nachbar"])
	_add_concept("world_generation", "world", ["WorldConfig", "WorldGenerator", "WorldBootstrap", "SeededLayout", "ChunkCoordinator", "PlanetCatalog", "PlanetDefinition", "SectorClassifier"], ["generate_catalog", "set_layout_seed", "reset_for_infinite_world", "resolved_target_planet_count", "is_infinite_world"], "Seeded world generation, catalogs, chunks and sectors", ["world", "welt", "layout", "seed", "catalog", "katalog", "chunk", "infinite", "unendlich", "sector", "generator"])
	_add_concept("planets", "planets", ["Planet", "PlanetNetwork", "PlanetNetworkUI", "PlanetGrid", "PlanetProcedural", "PlanetArrivalResolver", "SelectionService", "ContextMenuBuilder"], ["register_planet", "set_faction", "homeworld_for", "discover_planet", "scan_planet", "known_planets_of"], "Planet lifecycle, ownership, discovery and planet UI", ["planet", "planetoid", "homeworld", "heimwelt", "faction", "fraktion", "scan", "discover", "discovery", "capture"])
	_add_concept("combat_battle", "combat", ["FleetBattleSimulator", "ConquestSimulator", "BattleScene", "ConquestScene", "CombatReplay", "BattleContext"], ["simulate_battle", "simulate_conquest", "set_pending_battle_context"], "Fleet combat, conquest and replays", ["combat", "kampf", "battle", "schlacht", "conquest", "replay", "simulator", "conflict"])
	_add_concept("technology_research", "tech", ["TechDomain", "TechnologyMenu", "TechnologyCatalog", "TechnologyDefinition", "TechResearchView", "TechShipBuilderView", "TechPlanetView"], ["has_technology", "research_technology", "can_research_technology", "advance_research"], "Technology research, prerequisites and tech UI", ["technology", "technologie", "tech", "research", "forschung", "prerequisite", "voraussetzung", "trait"])
	_add_concept("ui_systems", "ui", ["PlanetPanel", "VaultBar", "MainMenu", "PauseMenu", "MessageFeed", "ModalCoordinator", "PaperDossier", "UIThemeConfig"], ["setup", "show_panel", "hide_panel", "refresh"], "Menus, panels, overlays, messages and theming", ["menu", "panel", "button", "overlay", "tooltip", "toast", "dossier", "pause", "theme"])
	_add_concept("scene_flow", "scenes", ["SceneDirector", "RunSession", "RunSaveData"], ["goto_scene", "snapshot_run", "restore_run", "request_new_run", "reconnect_world"], "Scene transitions, sessions and save/load", ["scene", "szene", "director", "transition", "wechsel", "save", "load", "speichern", "laden", "snapshot", "session"])
	_add_concept("testing_quality", "preflight", ["PreflightContext", "PreflightFixture", "MechanicRegistry", "ScenarioLoader", "ScenarioSnapshot"], ["constraint_name", "run", "boot_default", "check"], "Preflight constraints, fixtures and scenario coverage", ["preflight", "constraint", "test", "fixture", "scenario", "mechanic", "coverage"])
	...
	_add_concept("missions", "missions", ["TechResearchShipView"], ["get_research_missions", "queue_research_mission", "get_research_ship_records"], "Scout, collect and research missions", ["mission", "auftrag", "scout", "collect", "research mission"])
	_add_concept("background_visuals", "background", ["StarfieldBackground", "MapCamera", "MeteorConfig", "BackgroundConfig"], ["_draw_background", "_draw_stars", "_draw_nebulae"], "Starfield, meteors, fog and camera visuals", ["background", "hintergrund", "starfield", "meteor", "fog", "camera", "zoom", "pan"])
	...
	_add_concept("selection_input", "input", ["SelectionService", "MapCamera"], ["select_planet", "selected_planet", "deselect"], "Selection and input handling", ["select", "selekt", "click", "klick", "input", "context menu", "rechtsklick"])
