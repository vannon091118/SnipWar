...
python scripts/verify.py --scope-report
$GODOT_BIN --headless --path . --script res://scripts/preflight.gd --list
```

Der Verify-Treiber (`python scripts/verify.py`) bestimmt den Scope aus dem Staged-Set (contract_map.json) — Empty/unknown/duplicate muss fail-closed blockieren. Ohne Staged-Set läuft der vollständige Lauf. Die derzeitige Suite umfasst 44 Constraints. Der Pre-Commit-Hook verwendet den Verify-Treiber; Full-Preflight nur ohne Scope.

Wichtige Constraints umfassen `game_state_compatibility`, `concept_index`, `save_game_roundtrip`, `save_game_slots`, `mechanic_coverage`, `mcp_capture_contract` und Docs-Integrity. Die Narrative-Runtime ist entkoppelt (Home, `docs/DOKI_PIN.md`) und kein Preflight-Constraint mehr. Quelltext-Scanner überspringen Kommentarzeilen, damit Beispiele nicht als Code zählen.

## DOKI Interna (gepinntes Home)

DOKI ist ein reines Commit-Gate im **entkoppelten Home** (`C:/Users/Vannon/Desktop/doki/`, eigenes Projekt + eigenes git-Repo; Pinning: `docs/DOKI_PIN.md`): keine Imports, Autoloads oder Signale zu Spiel, MCP oder Agent-System. Das Spiel-Repo enthält weder Engine noch Runtime — nur die per-Repo-Artefakte unter `.doki/`. Die Schichten zeigen nur nach innen:
...
Die 14 Narratoren werden aus `n` gewählt, die Mood-/Struktur-Decodierung aus `j`. DOKI schreibt den technischen Commit nicht selbst als Geschichte; der Agent schreibt nach `.doki/narrator_body.md` Fließtext in der gezogenen Stimme.

Die zehn Verifier-Checks sind in `C:/Users/Vannon/Desktop/doki/README.md` kanonisch beschrieben. Checks 1–6 sind weich, Checks 7–10 hart; zusätzliche Verifier-Erweiterungen müssen Selfchecks und die README synchron halten. Check 10 beschränkt User-Dateien auf höchstens 30 pro Commit; auto-managed DOKI-Dateien zählen nicht.

## DOKI-Artefakt-Lifecycle

- `prepare`: Diff, Chain-Zustand, Composite, Arc- und Prompt-Kontext bestimmen (Aufruf: `bash C:/Users/Vannon/Desktop/doki/bin/doki prepare "<impuls>"` aus dem Repo-Root).
...
- `git commit -F .commit_msg.txt`: Hook führt Guard + AgentGate + Verify-Treiber aus (kein DOKI-Gate mehr — der Gate lebt im Home: `bash C:/Users/Vannon/Desktop/doki/bin/doki gate`).
...
Bei Änderungen an der Artifact-Policy müssen `C:/Users/Vannon/Desktop/doki/README.md`, `AGENTS.md`, Hook-Verhalten und Selfchecks gemeinsam geprüft werden. Runtime-Fehler dürfen den Commit niemals blockieren.
...
## Narrative Runtime (im Home)

Die verbindliche Spezifikation ist `C:/Users/Vannon/Desktop/doki/NARRATIVE_ENGINE_DESIGN.md`:
...
- Das NARRATIVE_RUNTIME_GATE (Home-Selfcheck `doki selfcheck` bzw. `doki gate`) prüft stdlib-only, Purity, deterministische IDs, Chain-Lücken, Schreibschutz, Idempotenz und Rebuild-Gleichheit.
...
- `docs/DOKI_PIN.md` — Pinning, Home-Layout, Upgrade-Pfad, Workflow.
- `C:/Users/Vannon/Desktop/doki/README.md` — DOKI-Flow, Composite, Checks, Recovery und Dateien.
- `C:/Users/Vannon/Desktop/doki/NARRATIVE_ENGINE_DESIGN.md` — Narrative-Runtime-Vertrag.
