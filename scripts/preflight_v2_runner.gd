...
##
## Usage: $GODOT_BIN --headless --path . --script res://scripts/preflight_v2_runner.gd [options]
...
# Constraints that are already validated by the V2Fixture boot — skip in Phase 2.
const SKIP_IN_PHASE2: Array[String] = ["scene_boot"]
...
func _init() -> void:
	...
	var ctx: PreflightV2Context = _V2Ctx.new(self)
	...
	if not scene_constraints.is_empty():
		...
		for entry in scene_constraints:
			...
			# Skip scene_boot in Phase 2 — fixture already booted by V2Fixture
			if entry["id"] in SKIP_IN_PHASE2:
				if ctx.verbose:
					print("[preflight-v2] Skipping %s (fixture already booted)" % entry["id"])
				continue
...
func _run_one(ctx: PreflightV2Context, entry: Dictionary) -> Array:
...
func _apply_fixture_to_ctx(ctx: PreflightV2Context, fixture: _V2Fixture) -> void:
...
======================================
Usage: godot --headless --path . --script res://scripts/preflight_v2_runner.gd [options]
...
func _print_summary(ctx: PreflightV2Context, total: int, passed: int, failed: int, total_ms: float) -> void:
	print("\n==================================================")
